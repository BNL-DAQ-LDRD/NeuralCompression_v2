...
from neuralcompress_v2.models.network3d import Encoder, BiDecoder
...
def get_args(description):
    ...
    parser.add_argument('--model-type',
                        choices = ('bcae++', 'bcae-ht'),
                        type    = str,
                        default = 'bcae++',
                        help    = ('Type of 3d BCAE models.\n'
                                   'In this release, we provide two choices for 3d BCAE models:\n'
                                   '    - BCAE++: A modification of the original BCAE;\n'
                                   '    - BCAE-HT: A modification of BCAE++ by using super small\n'
                                   '    number of output channels in each encoder block.\n'
                                   '    The HT stands for high-throughput.\n'
                                   '(default = bcae++).\n\n'))
...
def main():
    ...
    if model_type == 'bcae++':
        conv_features = (8, 16, 32, 32)
    else:
        conv_features = (2, 4, 4, 8)

    encoder = Encoder(conv_features = conv_features).to(device)
    decoder = BiDecoder().to(device)
    ...
    loss_fn = BCAELoss(clf_threshold = clf_threshold,
                       reg_loss      = reg_loss,
                       eps           = 1e-4 if half_training else 1e-8)
